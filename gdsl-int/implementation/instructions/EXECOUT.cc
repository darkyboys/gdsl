...
namespace gdsl_int {
    std::string EXECOUT(std::vector <std::string> commands, std::vector <std::vector <std::string>> &blocks){
        ...
        std::string error;
        if (blocks.size() < 1){
            error = "EXECOUT Can't find any top level memory allocation to store the STDOUT and STDERR of the program";
            return error;
        }
        // process code
        pid_t process = fork();
        ...
        if (process == 0){
            ...
            if (w_fd < 0) {
                perror("EXECOUT ERR: open failed");
                _exit(1);
            }
            dup2(w_fd, 1);
            close(w_fd);
            ...
            perror("EXECOUT ERR: execv failed");
            _exit(1);
        }
        else if (process < 0){
            error = "EXECOUT ERR: fork failed";
            return error;
        }
        
        int status;
        if (waitpid(process, &status, 0) == -1){
            error = "EXECOUT ERR: waitpid failed";
            return error;
        }
        std::ifstream ifile(file_path);
        if (!ifile.is_open()){
            error = "EXECOUT ERR: can not open the file ";
            return error;
        }
        std::string temp;
