...
// This is the EXECERR Instruction header for the gdsl-int
// C++ Headers
...
namespace gdsl_int {
    std::string EXECERR(std::vector <std::string> commands, std::vector <std::vector <std::string>> &blocks){
        bool is_defined = false;
        std::string error;
        if (blocks.size() < 1){
            error = "EXECERR Can't find any top level memory allocation to store the STDOUT and STDERR of the program";
            return error;
        }
        // process code
        pid_t process = fork();
        ...
        if (process == 0){
            ...
            if (w_fd < 0) {
                perror("EXECERR ERR: open failed");
                _exit(1);
            }
            dup2(w_fd, 2);
            close(w_fd);
            ...
            perror("EXECERR ERR: execv failed");
            _exit(1);
        }
        else if (process < 0){
            error = "EXECERR ERR: fork failed";
            return error;
        }
        
        int status;
        if (waitpid(process, &status, 0) == -1){
            error = "EXECERR ERR: waitpid failed";
            return error;
        }
        std::ifstream ifile(file_path);
        if (!ifile.is_open()){
            error = "EXECERR ERR: can not open the file ";
            return error;
        }
        std::string temp;
